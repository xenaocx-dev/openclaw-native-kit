...
远程状态快照：2026-09-13，通过当时登录 GitHub 账号的作者检索及具体 PR/issue 页面核对。下表状态不代表当前状态，范围也不包含其他账号。2026-09-18 仅补充 r2 本地候选证据，未联网刷新 PR/issue 状态。

## r2 新增的本地贡献线索

ACP 原生 prompt 已开始，但接管通知未贯通 ingress，可能被接管 watchdog 误判为卡住。r2 修复贯通四段回调，并让真实 Telegram 接管超时携带可识别的 Error 类型。此项与下文候选 1 都涉及任务已启动后的处理，但不是同一个修复：候选 1 针对 reply hook 失败后的普通 CLI 重路由；本项针对 ingress 接管计时与去重提交。

可复用 `overlay/tests/ingress-timeout.test.mjs` 的内存队列用例作为 issue/测试证据；未验证最新上游是否仍有问题，本轮不提交 PR/issue。正式贡献前须先复现和去重，并处理 `docs/release-r2.md` 中的接管回调失败 P1，不能把现有修复直接当成完整上游方案。
