...
function createChannelIngressDrain(options) {
	...
	const armStallWatchdog = (state) => {
		...
		state.stallTimer = setTimeout(() => {
			...
			let timeoutError;
			try {
				timeoutError = options.createAdoptionStallError?.({
					message,
				...
				log(`ingress drain: adoption stall error factory failed for event ${displayId}: ${formatError(error)}`);
			}
			if (!(timeoutError instanceof Error)) timeoutError = new Error(message);
			state.guillotined = true;
			clearStallTimer(state);
			log(message);
			try {
