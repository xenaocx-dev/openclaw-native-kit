...
function resolveClaudeProjectsDir(homeDir) {
	const explicitHomeDir = normalizeOptionalString(homeDir);
	if (explicitHomeDir) return path.join(resolveHistoryHomeDir(explicitHomeDir), CLAUDE_CONFIG_DIRNAME, CLAUDE_PROJECTS_DIRNAME);
	const configuredDir = normalizeOptionalString(process.env.CLAUDE_CONFIG_DIR);
	return configuredDir ? path.join(path.resolve(configuredDir), CLAUDE_PROJECTS_DIRNAME) : path.join(resolveHistoryHomeDir(), CLAUDE_CONFIG_DIRNAME, CLAUDE_PROJECTS_DIRNAME);
}
