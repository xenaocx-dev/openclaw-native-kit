...
async function closeAcpRuntimeForSession(params) {
	...
	for (const sessionKey of sessionKeys) {
		acpMeta = readAcpSessionMeta({
			...
			agentId: params.agentId,
			cfg: params.cfg,
			allowStaleLifecycleRevision: params.reason === "session-reset"
		});
		if (acpMeta) {
...
async function performGatewaySessionReset(params) {
	...
	return await runExclusiveSessionLifecycleMutation({
		...
		run: async () => {
			...
			if (hadExistingEntry && !resetSkipped && params.preserveConversationBindings !== true) await emitSessionUnboundLifecycleEvent({
